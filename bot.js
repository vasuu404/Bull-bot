...
async function beginSpinCountdown(channel) {
  ...
  // Each countdown number is a NEW message (never edited), with a large + bold number.
...
client.on('interactionCreate', async (interaction) => {
  ...
  const isMod = Boolean(member && isStaffMember(member));
  ...
  if (commandName === 'spin') {
    ...
    try {
      ...
      await interaction.editReply({ content: 'Spin giveaway started ✅' });
    } catch (error) {
      console.error('Spin command error:', error);
      await interaction.editReply({ content: 'Could not start the spin giveaway. Check the Railway logs for the error.' }).catch(() => {});
    }
    return;
  }
