...
const pendingCaptchas = new Map(); // userId -> { answer, expiresAt }
...
async function handleVerifyPanel(channel) {
  ...
  const embed = new EmbedBuilder()
    ...
    .setDescription('Click **Verify Me** and complete the quick CAPTCHA. After the correct answer, you will receive the Verified role.');
  await channel.send({ embeds: [embed], components: [row] });
}
...
function buildHelpEmbed() {
  return new EmbedBuilder()
    ...
    .addFields(
      ...
      { name: '/unban <user>', value: 'Search and select a banned user, then unban them (staff)' },
      ...
      { name: '/say <channel> <message>', value: 'Send a message to any text channel (staff)' }
    );
}
...
client.on('interactionCreate', async (interaction) => {
  if (interaction.isAutocomplete()) {
    ...
    try {
      const query = interaction.options.getString('user', false)?.toLowerCase() || '';
      ...
      const matches = bans
        .filter((ban) => {
          ...
          return !query || tag.includes(query) || username.includes(query) || ban.user.id.includes(query);
        })
        .first(25)
        .map((ban) => ({
      ...
      console.error('Could not load banned users for /unban:', e.message);
  ...
  if (commandName === 'unban') {
    ...
    try {
      ...
      if (!ban) return interaction.reply({ content: 'That user is not currently banned (or the ID is incorrect).', ephemeral: true });
      await guild.members.unban(userId, 'Unbanned by staff');
      return interaction.reply(`Unbanned **${ban.user.tag}** (<@${userId}>) ✅`);
    } catch (e) {
      console.error('Could not unban:', e.message);
      return interaction.reply({ content: 'Could not unban that user. Check my permissions and make sure I am above the relevant moderation hierarchy.', ephemeral: true });
    }
  }
  ...
  // /say — staff-only utility to send a plain message to any text-based channel.
  // The existing /announce command above is intentionally left unchanged.
  if (commandName === 'say') {
    if (!isMod) return interaction.reply({ content: 'Only staff can use this command.', ephemeral: true });
    ...
    try {
      ...
      console.error('Could not send /say message:', e.message);
      return interaction.reply({ content: 'I could not send the message there. Check my permissions for that channel.', ephemeral: true });
    }
  }
...
// ---------- BUTTON INTERACTIONS ----------
client.on('interactionCreate', async (interaction) => {
  // This handler also processes the verification CAPTCHA modal.
  ...
  if (interaction.customId === 'verify_button') {
    ...
    // Simple anti-bot CAPTCHA: generate a fresh addition problem and ask the
    // user to answer it in a Discord modal. Nothing is posted publicly.
    const a = Math.floor(Math.random() * 9) + 1;
    const b = Math.floor(Math.random() * 9) + 1;
    pendingCaptchas.set(interaction.user.id, { answer: a + b, expiresAt: Date.now() + 2 * 60 * 1000 });
  ...
  if (interaction.customId === 'verify_captcha_modal') {
    const captcha = pendingCaptchas.get(interaction.user.id);
    if (!captcha || Date.now() > captcha.expiresAt) {
      pendingCaptchas.delete(interaction.user.id);
      return interaction.reply({ content: '⏱️ CAPTCHA expired. Click **Verify Me** again to get a new one.', ephemeral: true });
    }

    const answer = Number.parseInt(interaction.fields.getTextInputValue('captcha_answer').trim(), 10);
    if (!Number.isInteger(answer) || answer !== captcha.answer) {
      pendingCaptchas.delete(interaction.user.id);
      return interaction.reply({ content: '❌ Incorrect CAPTCHA. Click **Verify Me** again and try a new one.', ephemeral: true });
    }
    ...
    try {
      ...
      console.error('Could not add verified role:', e.message);
      return interaction.reply({ content: 'I could not give you the Verified role. Please contact staff.', ephemeral: true });
    }
    ...
    return interaction.reply({ content: '✅ CAPTCHA passed! You are now verified and can access the server 🎉', ephemeral: true });
  }
...
client.on('messageCreate', async (message) => {
  ...
  if (command === 'unban') {
    ...
    if (!userId || !/^\d{17,20}$/.test(userId)) return message.reply('Usage: `!unban <user_id>`');
    const ban = await message.guild.bans.fetch(userId).catch(() => null);
    if (!ban) return message.reply('That user is not currently banned (or the ID is incorrect).');
    await message.guild.members.unban(userId, 'Unbanned by staff').catch(() => message.reply('Could not unban — check my permissions.'));
    return message.reply(`Unbanned ${ban.user.tag} (<@${userId}>) ✅`);
  }
