...
async function beginSpinCountdown(channel) {
  ...
  // Testing mode: no @here ping.
  await channel.send(`🎰 **${spinConfig.prize}** spin giveaway is about to start!`);
...
function renderSlotGrid(ids, guild) {
  ...
  return [
    '```text',
    '┌────────────────┬────────────────┬────────────────┐',
    `│ ${names[0].padEnd(14)} │ ${names[1].padEnd(14)} │ ${names[2].padEnd(14)} │`,
    '├────────────────┼────────────────┼────────────────┤',
    `│ ${names[3].padEnd(14)} │ ${names[4].padEnd(14)} │ ${names[5].padEnd(14)} │`,
    '├────────────────┼────────────────┼────────────────┤',
    `│ ${names[6].padEnd(14)} │ ${names[7].padEnd(14)} │ ${names[8].padEnd(14)} │`,
    '└────────────────┴────────────────┴────────────────┘',
    '```',
  ].join('\n');
}
...
async function runSpinShuffle(channel) {
  ...
  const winnerId = pool[Math.floor(Math.random() * pool.length)];
  const rounds = Math.max(8, spinConfig.spinRounds || 8);

  // 3x3 grid with rapid edits and blur characters for a visible shuffle.
  const slotMsg = await channel.send({
    content: `🎰 **SPINNING...**\n${renderSlotGrid(shuffle3x3(pool), channel.guild)}`,
  });
  ...
  await slotMsg.edit({
    content: `🎉 **WINNER!**\n${renderSlotGrid(Array(9).fill(winnerId), channel.guild)}`,
  }).catch(() => {});
  ...
  const claimEmbed = new EmbedBuilder()
    ...
    .setTitle('🎉 You won!')
    .setDescription(
      `<@${winnerId}> you won **${spinConfig.prize}**!\n\nClick **Open Ticket** to claim your prize. You have **1 minute** to respond. If you do not respond, this spin will automatically respin.`
    );
  ...
  spinClaim.timeoutHandle = setTimeout(async () => {
    if (spinClaim && !spinClaim.resolved) {
      ...
      await channel.send(`⏱️ <@${winnerId}> did not respond in time. 🔄 **Respinning...**`);
      spinParticipants = new Set();
      spinClaim = null;
      // Timeout does not consume one of the configured spins.
      await beginSpinCountdown(channel);
    }
  }, 60000);
}
...
client.on('interactionCreate', async (interaction) => {
  ...
  if (interaction.customId === 'claim_noopen') {
    ...
    if (spinClaim.timeoutHandle) clearTimeout(spinClaim.timeoutHandle);
    await interaction.update({ components: [] }).catch(() => {});
    await interaction.followUp('Okay, no ticket will be opened. 🔄 Respinning automatically...');

    const channel = interaction.channel;
    spinParticipants = new Set();
    spinClaim = null;
    // Declining does not consume one of the configured spins.
    await beginSpinCountdown(channel);
    return;
  }
});
